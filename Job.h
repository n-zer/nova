...
namespace Nova {
	...
	template <typename Callable, typename ... Ts>
	internal::SimpleJob<Callable, Ts...> MakeJob(Callable callable, Ts... args) {
		//To-do, insert SFINAE check for () operator on callable
		return internal::SimpleJob<Callable, Ts...>(callable, args...);
	}

	template <typename Callable, typename ... Ts>
	internal::BatchJob<Callable, Ts...> MakeBatchJob(Callable callable, unsigned start, unsigned end, Ts... args) {
		//To-do, insert SFINAE check for () operator on callable
		return internal::BatchJob<Callable, Ts...>(callable, start, end, args...);
	}

	template <typename Obj, typename ... Ts>
	internal::SimpleJob<internal::MemberFunctionWrapper<Obj, Ts...>, Ts...> MakeJob(void(Obj::*func)(Ts...), Obj obj, Ts... args) {
		return internal::SimpleJob<internal::MemberFunctionWrapper<Obj, Ts...>, Ts...>({ obj,func }, args...);
	}

	template <typename Obj, typename ... Ts>
	internal::BatchJob<internal::MemberFunctionWrapper<Obj, Ts...>, Ts...> MakeBatchJob(void(Obj::*func)(Ts...), Obj obj, unsigned start, unsigned end, Ts... args) {
		return internal::BatchJob<internal::MemberFunctionWrapper<Obj, unsigned, unsigned, Ts...>, Ts...>({ obj,func }, start, end, args...);
	}
}
